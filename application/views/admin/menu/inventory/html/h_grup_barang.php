...
$(document).ready(function() {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=k_grup_barang&columns=,kode,nama,brand_id&jwhere=brand_id&fildjoins=,m_brand.nama as Brands&joins=m_brand&exports=m_brand",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $(nTd).attr("id", id);
                    $(nTd).editable(
                        "<?php echo site_url('admin/update_grid/k_grup_barang'); ?>", {
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $(nTd).attr("id", id);
                    $(nTd).editable(
                        "<?php echo site_url('admin/update_grid/k_grup_barang'); ?>?table=m_brand", {
    ...
    $("#btnAdd").button().click(function() {
        ...
        $.auto_number("k_grup_barang", "2", "#txtKode");
        $("#cmbBrand").empty();
        $.ajax({
            type: "POST",
            dataType: 'json',
    ...
    $('#add-data-ctgr').submit(function(e) {
        ...
        if ($('#add-data-ctgr').valid()) {
            $.ajax({
                type: "POST",
                dataType: 'json',
                url: "<?php echo site_url('admin/create/k_grup_barang'); ?>",
                data: {
                    kode: $('#txtKode').val(),
                    nama: $("#txtNama").val(),
