...
$(document).ready(function() {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=w_menu&columns=,menu,id&jwhere=link_id&fildjoins=,m_seo.title,m_seo.icon,m_seo.deskripsi,m_seo.keyword,m_seo.id as seoId,m_seo.author&joins=m_seo&exports=m_seo",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
    ...
    $("#btnAddSub").button().click(function() {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=w_menu",
                ...
                success: function(ret) {
                    if (ret.error == false) {
                        $.ajax({
                            type: "POST",
                            dataType: 'json',
                            url: "<?php echo site_url('admin/create/w_menu'); ?>",
                            data: {
                                menu: $("#txtMenu").val(),
                                link_id: ret.id,
                                status: 'Y',
    ...
    $("#edit-data").submit(function(e) {
        ...
        if ($('#edit-data').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('admin/update/w_menu'); ?>",
                data: {
                    id: selectedId,
                    name: $("#txtMenu_e").val(),
                    link_id: seoId,
