...
$(document).ready(function() {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=w_submenu&columns=,name&jwhere=link_id&fildjoins=,m_seo.title,m_seo.icon,m_seo.deskripsi,m_seo.keyword,m_seo.id as seoId,m_seo.author&joins=m_seo&exports=m_seo",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $($(nTd).children()[0]).button().click(function() {
                        selectedId = oData.id;
                        $("#txtMenu_e").val(oData.name);
                        $("#txtTitle_e").val(oData.title);
                        $("#txtDeskripsi_e").val(oData.deskripsi);
                        $("#txtPatch_e").val(oData.html_patch);
                        $("#txtUrl_e").val(oData.url);
            ...
            success: function(data) {
                if (data == '') {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $("#cmbMenu").append("<option value='" + data[i].id + "'>" + data[i]
                            .menu + "</option>");
                    }
                }
            }
                ...
                success: function(ret) {
                    if (ret.error == false) {
                        $.ajax({
                            type: "POST",
                            dataType: 'json',
                            url: "<?php echo site_url('admin/create/w_submenu'); ?>",
                            data: {
                                name: $("#txtMenu").val(),
                                link_id: ret.id,
                                status: 'Y',
    ...
    $("#edit-data").submit(function(e) {
        ...
        if ($('#edit-data').valid()) {
            ...
            $.ajax({
                type: "POST",
                dataType: 'json',
                url: "<?php echo site_url('admin/update/w_submenu'); ?>",
                data: {
                    id: selectedId,
                    menu: $("#txtMenu_e").val(),
                    link_id: seoId,
                    html_patch: $("#txtPatch_e").val().replaceAll("/", "-"),
                    url: $("#txtUrl_e").val(),
                    date: $date
                },
                        ...
                        $.ajax({
                            ...
                            success: function(msg) {
                                if (msg.error == false) {
                                    ...
                                    var title = "Edit";
                                    var label = "Data Table..";
                                    alert_info(title, label, msg.message);
                                    ...
                                    var title = "Edit";
                                    var label = "Data Table..";
                                    alert_warning(title, label, msg.message);
                                    $("#frm-edit-menu").modal("hide");
                                }
                                Edit = false;
                            }
    ...
    $('#btn-prosess').button().click(function() {
        $.ajax({
            url: "<?php echo site_url('admin/aktive/w_submenu'); ?>/" + fildeselect + "/" +
                selectedId + "/" + active,
            success: function(data) {
                $("#tblData").dataTable().fnDraw();
