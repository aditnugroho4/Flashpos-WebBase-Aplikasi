...
$(document).ready(function() {
    ...
    $.edit_content = function(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#txtQuote_e").val(data[0].quote);
                $("#txtImg_e").val(data[0].img);
                $("#txtReviewed_e").val(data[0].reviewed);
                $("#e_img").attr('src', '<?= base_url()?>' + data[0].img);
    ...
    $("#cmbImg").change(function() {
        ...
        if (Edit == true) {
            ...
            $("#txtImg_e").val(link_img);
            $("#e_img").attr('src', '<?= base_url()?>' + link_img);
        } else {
            $("#txtIcon").val(link_img);
            $("#txtIcon").attr('disabled', 'disabled');
            $("#txtImg").val(link_img);
            $("#imgAdd").attr('src', '<?= base_url()?>' + link_img);
        }
    ...
    $.edit_popup = function(fild, id, status) {
        if (status == 'Y') {
            status = 'N';
        } else if (status == 'N' || status == null) {
            status = 'Y';
        }
        Swal.fire({
            icon: 'info',
            title: 'Edit Kontent',
            text: 'Setting Ke popup halaman utama',
            confirmButtonText: "OK",
            focusConfirm: true
        }).then((result) => {
            if (result.value) {
                $.ajax({
                    url: "<?php echo site_url('admin/aktive/w_post_artikel'); ?>/" +
                        fild + "/" +
                        id + "/" + status,
                    success: function(data) {
                        $_pagination_get();
...
function Templating(data) {
    ...
    for (var i = 0, len = data.length; i < len; i++) {
        let popups = "";
        if (data[i].items.popup == 'Y') {
            popups = '<i class="fas fa-check btn badge badge-success" onclick="$.edit_popup(\'\popup\',' + data[i]
                .items.id + ',\'' + data[i].items.popup + '\')"> popup</i>';
        } else if (data[i].items.popup == 'N' || data[i].items.popup == null) {
            popups = '<i class="fas fa-eye btn badge badge-info" onclick="$.edit_popup(\'\popup\',' + data[i].items
                .id + ',\'' + data[i].items.popup + '\')"> Set Popup</i>';
        }
        $html += '<li>' +
            '<span class="handle">' +
            ...
            ' Comment</small>' +
            '<div class="tools">' + popups +
            '<i class="fas fa-edit badge badge-warning" onclick="$.edit_content(' + data[i].items.id +
            ');" > Edit</i>' +
            '</div>' +
            '</li>';
    }
