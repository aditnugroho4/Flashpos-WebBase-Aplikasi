...
$(document).ready(function() {
    ...
    $("#tblData-setting").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=w_post_images&columns=,w_post_images.name,w_post_images.date,m_kategory.name,size,ctgr_id&jwhere=ctgr_id&fildjoins=,m_kategory.name as nama_katagory&joins=m_kategory&exports=m_kategory",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    if (oData.img == null) {
                        htmlx =
                            "<div class='from-group'><img class='img-thumbnail' width='65' src='<?php echo base_url('asset/admin/dist/default-150x150.png'); ?>'></div>";
                        $(nTd).html(htmlx);
                    } else {
                        htmlx = "<div class='from-group thumbnail'><img id='img-" + oData.id +
                            "' class='img-thumbnail'width='65' src='<?php echo base_url(); ?>/" +
                            oData.img + "' alt='' ></div>";
                        $(nTd).html(htmlx);
                    }
                }
            ...
            success: function(data) {
                if (data == '') {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $("#cmbLink-upd").append('<option value="' + data[i].id + '">' +
                            data[i].name + '</option>');
                    }
                }
            }
