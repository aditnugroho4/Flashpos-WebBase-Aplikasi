...
$(document).ready(function() {
    ...
    $("#edit-Product").submit(function(e) {
        ...
        if ($('#edit-Product').valid()) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('admin/update/w_post_product'); ?>",
                data: {
                    id: selectedId,
                    name: $("#txtNama").val(),
                    label: $("#txtLabel").val(),
    ...
    $.edit_content = function($id) {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=w_post_product&select=id&id=" +
                $id,
            success: function(data) {
                $("#txtNama").val(data[0].name);
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data) {
                    ...
                    $("#e_img").attr('src', '<?= base_url()?>' + data[0].icon);
                    Edit = true;
                }
            }
        });
    ...
    $("#btnUpload").button().click(function() {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=w_post_product",
            success: function(data) {
                if (data == '') {
...
function $_pagination_get() {
    $('#pagination').pagination({
        dataSource: '<?= site_url('admin/json_load_data'); ?>?table=w_post_product',
        locator: 'data',
        pageSize: 10,
        pageRange: null,
