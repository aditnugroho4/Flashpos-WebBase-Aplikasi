...
$(document).ready(function() {
    ...
    $("#btnEditProf").button().click(function() {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=m_confiq",
    ...
    $('.btn-load').button().click(function() {
        if (Content) {
            CKEDITOR.instances['txtContent'].insertHtml(Content);
            $(this).hide();
        } else {
            CKEDITOR.instances['txtContent'].insertHtml("Belum Ada Konten Yang di Muat");
        }
    });
            ...
            for (var i = 0; i < Data.length; i++) {
                if (Data[i].name == 'Title') {
                    $.ajax({
                        type: "POST",
                        dataType: 'json',
                        url: "<?php echo site_url('admin/update/m_confiq'); ?>",
                        data: {
                            id: Data[i].id,
                            variabel: $("#txtTitle").val()
                        },
    ...
    $("#edit-post-profil").submit(function(e) {
        e.preventDefault();
        if ($('#edit-post-profil').valid()) {
            $('.load-ding').append(
                '<div class="overlay text-center"><i class="fas fa-2x fa-sync-alt fa-spin"></i></div>'
            );
            if (CKEDITOR.instances['txtContent'].getData().length > 0) {
                for (var i = 0; i < Data.length; i++) {
                    if (Data[i].name == 'Profil') {
                        $.ajax({
                            type: "POST",
