...
$(document).ready(function() {
    $('.mn-1').addClass('bg-green');

    $('.dropify,#upl-upd-foto-prof').dropify({
        messages: {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=w_post_banner&columns=,text_title,text_judul,text_deskripsi,btn_id&jwhere=btn_id&fildjoins=,w_post_link_button.name,w_post_link_button.link_target&joins=w_post_link_button&exports=w_post_link_button",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    if (oData.foto == null) {
                        ...
                        htmlx =
                            "<div class='from-group thumbnail'><img class='img-thumbnail'width='65' height='65' src='<?= base_url('asset/images/product/gallery'); ?>/" +
                            oData.foto + "' alt='' ></div>";
                        $(nTd).html(htmlx);
                    }
                }
            },
            {
                ...
                "bSortable": false,
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    $(nTd).html("<button class='btn btn-xs bg-pink btn-sm'>Edit</button>");
                    $($(nTd).children()[0]).button().click(function() {
    ...
    $("#btnAdd").button().click(function() {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select_2');?>?table=w_post_product&select=status&val=Y",
    ...
    $("#add-data-slider").submit(function(e) {
        ...
        if ($('#add-data-slider').valid()) {
            ...
            $.ajax({
                ...
                data: {
                    text_title: $("#txtTitle").val(),
                    text_judul: $("#txtJudul").val(),
                    text_deskripsi: $("#txtDeskripsi").val(),
                    btn_id: $("#cmbLink option:selected").val(),
                    status: 'N',
                    date: $date,
                    prod_id: $("#cmbKategory option:selected").val(),
                    user_id: $user
                },
            ...
            try {
                ...
                if (idFoto) {
                    var confiq = {
                        ...
                        sizeW: 500,
                        tbL: "w_post_banner",
                        path: "asset-images-product-gallery"
                    };
