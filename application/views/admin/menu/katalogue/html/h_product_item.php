...
$(document).ready(function() {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=b_product_item&columns=,b_product_item.nama,deskripsi,harga,grup_id,b_product_grup.nama&jwhere=grup_id&fildjoins=,b_product_grup.nama as Grup&joins=b_product_grup&exports=b_product_grup",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $(nTd).attr("id", id);
                    $(nTd).editable(
                        "<?php echo site_url('admin/update_grid/b_product_item'); ?>?table=b_product_grup", {
                            loadurl: "<?php echo site_url('admin/get_editable_select'); ?>?table=b_product_grup&filds=nama&select=" +
                                oData.grup_id,
                            type: "select",
                            submit: "OK",
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $(nTd).editable(
                        "<?php echo site_url('admin/update_grid/b_product_item'); ?>", {
                            "callback": function(sValue, y) {
                                /* Redraw the table from the new data on the server */
                                $("#tblData").dataTable().fnDraw();
                            }
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $($(nTd).find('.btn-group').children()[0]).button().click(function() {
                        ...
                        $("#txtHargaModalE").val(Globalize.format(oData.harga,
                            "c"));
                        $("#txtMarginE").val(oData.margin);
                        $("#txtProfitE").val(Globalize.format(oData.profit,
                            "c"));
                        $("#txtPriceE").val(Globalize.format(oData.price,
                            "c"));
    ...
    $('#Add-Barang').click(function() {
        ...
        $.auto_number("b_product_item", "4", "#txtKdBarang");
    ...
    $("#add_data_barang").submit(function(e) {
        ...
        if ($('#add_data_barang').valid()) {
            ...
            $.ajax({
                type: "POST",
                dataType: 'json',
                url: "<?php echo site_url('admin/create/b_product_item'); ?>",
                data: {
                    ...
                    harga: Globalize.parseInt($("#txtHargaModal").val()),
                    margin: $("#txtMargin").val(),
                    profit: Globalize.parseInt($("#txtProfit").val()),
                    price: Globalize.parseInt($("#txtPrice").val())
                },
    ...
    $('#edit_data_barang').submit(function(e) {
        ...
        if ($('#edit_data_barang').valid()) {
            $('.load-Barang').append(
                '<div class="overlay"><i class="fas fa-2x fa-sync-alt fa-spin"></i></div>');
            $.ajax({
                url: "<?php echo site_url('admin/update/b_product_item');?>",
                type: "post",
                dataType: "JSON",
                data: {
                    id: selectedId,
    ...
    $('#btnChange').button().click(function() {
        $.ajax({
            url: "<?php echo site_url('admin/aktive/b_product_item'); ?>/" + fildeselect + "/" +
                selectedId + "/" + active,
            success: function(data) {
                $("#tblData").dataTable().fnDraw();
                $("#dlgAlert").modal("hide");
...
<div class="col-md-12">
    <div class="card card-outline card-yellow load-Barang">
        <div class="card-header">
            <h3 class="card-title col-lg-4 col-sm-12 mb-2">Data Items Satuan</h3>
        </div>
        <div class="card-body ">
