...
$(document).ready(function() {
    ...
    $("#tblData").dataTable({
        ...
        "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=m_datapegawai&columns=,nip,m_datapegawai.nama,pendidikan,jabatan,profesi,unit_id,divisi_id,m_unit.name,m_struktur.name&jwhere=divisi_id,unit_id,sosial_id&fildjoins=,m_struktur.name as Divisi,m_unit.name as Unit,m_datasosial.nik&joins=m_struktur,m_unit,m_datasosial&exports=m_struktur,m_unit,m_datasosial",
        "aoColumns": [{
                "sTitle": "#",
                "mData": "no",
                "bSortable": false,
    ...
    $('#AddData').click(function() {
        ...
        $.auto_number('m_datapegawai', 3, '#txtNip');
            ...
            success: function(data) {
                if (data == '') {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        $(tag).append("<option value='" + data[i].id + "'>" + data[i].name +
                            "</option>");
                    }
                    if (idSelected != false) {
                        $(tag).val(idSelected);
