...
$(document).ready(function() {
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    ...
                    $($(nTd).find('.btn-group').children()[0]).button().click(function() {
                        selectedId = oData.id;
                        window.open(
                            "<?= site_url('Kasir/print_penerimaan'); ?>?id=" +
                            $.base64.encode(selectedId), 'Data Penerimaan',
    ...
    $.get_detail_permintaan = function(id) {
        ...
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=k_penerimaan&select=faktur_id&id=" +
                id,
    ...
    $("#submit_prosses").button().click(function() {
        if ($("#tblListStock").dataTable().fnGetData().length > 0) {
            ...
            objItems.TableJual = $("#tblListStock").dataTable().fnGetData();
            var data = $.base64.encode(JSON.stringify(objItems));
            data = data.replaceAll(".", "^");
            data = data.replaceAll("+", "-");
            data = data.replaceAll("/", "_");
            let Url = "";
            if (objItems.kdAkun == '01') {
                Url = "<?php echo site_url('kasir/prosess_stock_penjualan'); ?>";
            } else {
                Url = "<?php echo site_url('kasir/prosess_stock'); ?>";
            }
            $.ajax({
                type: "POST",
                data: "data=" + data,
            ...
            "aoColumns": [{
                ...
                },
                {
                    ...
                    "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                        ...
                        if (oData.status == null) {
                            $($(nTd).children()[0]).find("span").html(
                                "&nbsp;Segera Prosess");
                        } else if (oData.status == 'Y') {
                            $($(nTd).children()[0]).find("div").attr("class",
                                "btn btn-sm btn-info");
                            $($(nTd).children()[0]).find("i").html("&nbsp;Jual");
                            $.loop("#i-color-" + oData.id);
                        } else if (oData.status == 'N') {
                            $($(nTd).children()[0]).find("div").attr("class",
                                "btn btn-sm btn-danger");
                            $($(nTd).children()[0]).find("span").html("&nbsp;Di Tolak");
                        }
                    }
                }
            ]
                        ...
                        <div class="col-lg-12">
                            ...
                            <div class="card-footer">
                                <div class="form-group">
                                    <button type="button" id="submit_prosses" class="btn btn-brown float-right"><i
                                            class="fa fa-send"></i> Prosses</button>
                                    <button type="button" id="submit_batal" class="btn btn-brown float-right mr-1"><i
                ...
                <div class="tab-pane" id="RincianStock">
                    <div class="row">
                        <div class="col-lg-12">
                            ...
                            <div class="card-footer">
                                <div class="form-group">
                                    <button type="button" id="submit_print" class="btn btn-brown float-right mr-1"><i
