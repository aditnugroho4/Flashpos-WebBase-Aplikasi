...
$(document).ready(function() {
        ...
        "aoColumns": [{
            ...
            },
            {
                ...
                "fnCreatedCell": function(nTd, sData, oData, iRow, iCol) {
                    if (oData.action == 'S') {
                        htmlx =
                            "<button style='background: #4BE807;background: -webkit-gradient(linear, left top, left bottom, from(#4BE807), to(#C4EB1A));'>Perosess</button>";
                        ...
                        htmlx =
                            "<button style='background: #EB1A1A; background: -webkit-gradient(linear, left top, left bottom, from(#EB1A1A), to(#EB891A)); color:#FBFAFA;'>Selesai</button>";
                        $(nTd).html(htmlx);
                        $($(nTd).children()[0]).button().click(function() {
                            var id = "action-" + oData.id;
                            $(nTd).attr("id", id);
                        ...
                        htmlx =
                            "<button style='background: #EB1A1A; background: -webkit-gradient(linear, left top, left bottom, from(#EB1A1A), to(#EB891A)); color:#FBFAFA;'>Meunggu</button>";
                        $(nTd).html(htmlx);
                        $($(nTd).children()[0]).button().click(function() {
                            var id = "action-" + oData.id;
                            $(nTd).attr("id", id);
    ...
    $("#data-baru-form").dialog({
        ...
        buttons: {
            Kirim: function() {
                if ($("#txtKomplen").val().length > 0 && $("#cmbInstalasi option:selected").val()
                    .length > 0 && $("#cmbKategory option:selected").val().length > 0) {
                    $.ajax({
                        type: "POST",
    ...
    $("#cmbInstalasi").change(function() {
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: "<?php echo site_url('admin/multi_select');?>?table=t_komplen_kategory&select=instalasi_id&id=" +
                $(this).val(),
            success: function(data) {
                if (data == '') {
                    var options = "<option value=''> -- No Result -- </option>";
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        options += "<option value='" + data[i].id + "'>" + data[i].nama +
                            "</option>";
                    }
