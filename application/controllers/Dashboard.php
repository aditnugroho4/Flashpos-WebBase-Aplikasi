...
private $user;
	...
	public function get_all_member(){
		R::setStrictTyping(false);
			...
			try{
				...
				$row=0;
				$view=0;
					$sql =R::getAll("SELECT * FROM m_user_member ORDER BY id DESC LIMIT ".$start.",".$limit."");
					if($sql){						
	...
	public function json_get_client(){
		...
		try {
			...
			if($sql){
				foreach ($sql as $key => $value) {
					$chat = R::findOne('m_user_chat_replay','member_id=? AND replay_id IS NULL AND activ IS NULL ORDER BY date DESC',array($value['id']));
					$newMessage=R::count('m_user_chat_replay','member_id=? AND replay_id IS NULL AND activ IS NULL ORDER BY date DESC',array($value['id']));
					$message = "";
					if($chat){
						$message = $chat->export();
					}else{
	...
	public function chat_send_message(){
		...
		try{
			if($this->input->post()){
				...
				$status = R::getAll('SELECT * FROM m_user_chat_replay WHERE member_id= ? AND replay_id IS NULL AND activ IS NULL',array($this->input->post('clientId')));
						if($status){
	...
	public function json_chat_notif(){
		...
		$date = R::isoDateTime();
		try{
			$Message = R::count("m_user_chat_replay","replay_id IS NULL GROUP BY member_id AND activ IS NULL AND date like ?",array(substr($date,0,7).'%'));
			...
			$userClient = R::count("m_user_member");
			...
			$client_notif = '<li class="nav-item dropdown">
							...
							$user_notif ='<li class="nav-item dropdown">
								...
								<div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">';
							$user_admin = R::getAll("SELECT * FROM m_user where auth='Y' ORDER BY date desc LIMIT 0,5"); 
							$url='';
							$note ='';
	...
	public function clear_chat($id){
		...
		$date = R::isoDateTime();
		try{
			$status = R::getAll('SELECT * FROM m_user_chat_replay WHERE member_id= ? AND activ IS NULL ',array($id));
						if($status){
							foreach ($status as $key => $value) {
								$read = R::load('m_user_chat_replay',$value['id']);
								$read->activ ='N';
