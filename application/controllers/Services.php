...
class Services extends CI_Controller {
    ...
    public function index($url=null){
		if($url==null){
			$data=array('user'=>R::load('m_user_member',$this->Auth['user']),
					...
					'content'=>str_replace('-','/','portal-menu-content-html-h_Layanan'),
    ...
    public function detail($id=null){
		...
		$sql =R::findOne("w_post_product","status ='Y' AND name=?",array($link));
		$seo=null;
		$prd=null;
		if($sql){
			$seo=$sql->link_id;
		...
		$data=array('user'=>R::load('m_user_member',$this->Auth['user']),
					'Menu'=>$this->get_data_menu(),
					'content'=>str_replace('-','/','portal-menu-product-html-h_Layanan_detail'),
					'Product'=>$this->get_data_product($prd),	
					'Layanan'=>$this->get_data_product(null),	
	...
	public function get_data_product($id=null){
		...
		try {
			if($id==null){
				...
				$data =R::findOne("w_post_product","status='Y' AND id=?",array($id));
				if($data){
					$arr['nama']=$data->name;
