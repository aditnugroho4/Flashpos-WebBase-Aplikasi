...
class Home extends CI_Controller {
	...
	public function get_data_banner(){
		...
		try{
			...
			foreach($data as $key=>$value){
				$arr['data'][]=array('Title'=>$value['text_title'],
									'Judul'=>$value['text_judul'],
									'Desc'=>$value['text_deskripsi'],
									'Img'=>$value['foto'],
									'Ctgr'=>$value['prod_id'],
									'Link'=>R::load('w_post_link_button',$value['btn_id'])->export()
								);
			}
		}catch (Exception $e) {
			$arr['error'] =true;
	...
	public function get_data_gallery(){
			...
			$arr =  array();
			try{
				$sql=R::getAll("SELECT * FROM w_post_product WHERE product='Y';");
				$count=0;
				$row=0;
				$view=0;
					...
					if($sql1){						
						foreach($sql1 as $key=>$value){
							$arr['images'][]=array('img'=>$value['img'],
								'nama'=> $value['name'],
								'lokasi'=> $value['lokasi'],
								'label'=> R::load('w_post_product',$value['ctgr_id'])->name,
		...
		try {
			...
			$blogAll = R::getAll("SELECT * FROM w_post_artikel ORDER BY id DESC Limit 0,10");
				foreach($blogAll as $val){
					$arr['All'][]=array($val,'seo'=>R::load('m_seo',$val['link_id'])->export(),'ctgr'=>R::load('w_post_ctgr_blog',$val['ctgr_id'])->nama);
				}
		}catch (Exception $e) {
			$arr['error'] =true;
			$arr['code'] =201;
	...
	public function get_data_product($id=null){
			...
			try {
				if($id==null){
					$data =R::getAll("SELECT * FROM w_post_product WHERE `status`='Y';");
					foreach($data as $val){
						$arr['data'][]=array($val);
					}
				}else{
					$data =R::findOne("w_post_product","status='Y' AND `name` LIKE ?",array($id.'%'));
					if($data){
						$arr['nama']=$data->name;
	...
	public function json_load_ulasan(){
		...
		$arr=array();
		try{
			$prod = R::getAll("SELECT * FROM w_post_product WHERE status='Y'");
			if($prod){
				foreach($prod as $i=> $items){
					$sql = R::getAll("SELECT uc.*,um.first_name,um.last_name,um.foto,um.alamat FROM m_user_ulasan uc INNER JOIN m_user_member um ON um.id=uc.member_id WHERE uc.product_id=? ORDER BY uc.id DESC LIMIT 0,3",array($items['id']));
					$ulasan=array();
					if($sql){
						foreach($sql as $i=> $val){
							$ulasan[]= $val;
					...
					$arr["data"][]=array('product'=>$items,'ulasan'=>$ulasan);
				}
				$arr['code']='100';
				$arr['error']='false';
				$arr['message']="Success..";
	...
	public function json_load_dokter(){
		R::setStrictTyping(false);
			$start = 0;
			$limit = 0;
			if($this->input->post('limit')){
			...
			$arr =  array();
			try{
				$sql=R::getAll("SELECT * FROM m_dokter GROUP BY kategory");
				$count=0;
				$row=0;
				$view=0;
				foreach($sql as $key=>$v){
					$arr['data'][]=array('name'=>$v['kategory'],'css'=>$v['id']);
					$count += R::count("m_dokter","status='Y'");
					$sql1 =R::getAll("SELECT * FROM m_dokter WHERE kategory=? AND status='Y' ORDER BY id DESC LIMIT ".$start.",".$limit."",array($v['kategory']));
					if($sql1){						
						foreach($sql1 as $key=>$value){
							$arr['dokter'][]=$value;
							$row +=1;
	...
	public function get_profile_dokter(){
			R::setStrictTyping(false);
			$ret = array();
			$id=$this->input->get('id');
			try{
				$sql=R::load("m_dokter",$id);
				if($sql){
					$ret['data']=array('id'=>$sql->id,
					'nama'=>$sql->nama_dokter,
					'keterangan'=>$sql->keterangan,
					'pendidikan'=>$sql->pendidikan,
					'img_id'=>$sql->img_id,
					'jadwal'=>R::load("w_post_foto",$sql->img_id)->name,
					'foto'=>$sql->foto,
			...
			$this->load->view('portal/menu/product/html/h_Profile_dr',$ret);
		}	
		// Function Triger
	...
	public function sys_user_monitoring($table){
		...
		$host = exec('hostname');
		$host  = trim($host); //remove any spaces before and after
		...
		$data['Mac'] = $MAC;
		$data['host'] = $host;
		$data['ipAddress'] = $this->ipCheck();
		$data['ipLocation'] = $this->locCheck($data['ipAddress'])->geoplugin_city;
		$data['sysinfo'] = $this->sys_get_info();
		$data['browser'] = str_replace('/',' ',$_SERVER['HTTP_USER_AGENT']);
		if($this->input->post())
		{
			$cek = R::findOne('n_web_monitoring','macaddress =? OR ipaddress=?',array($MAC,$data['ipAddress']));
			if(!$cek){
			foreach ($this->input->post() as $column => $value) {
					$bean->$column = $value;
					}
					$bean->macaddress =$MAC;
					$bean->ipaddress =$data['ipAddress'];
					$bean->lokasi = $data['ipLocation'];
					$bean->devices = $host;
					$bean->browser =$data['browser'];
					$bean->sysinfo =$data['sysinfo'];
					$id = R::store($bean);
			}else{
				foreach ($this->input->post() as $column => $value) {
					$cek->$column = $value;
					...
					$cek->macaddress =$MAC;
					$cek->ipaddress =$data['ipAddress'];
					$cek->lokasi = $data['ipLocation'];
					$cek->devices =$host;
					$cek->browser =$data['browser'];
					$cek->sysinfo =$data['sysinfo'];
					$id = R::store($cek);
			}
		}
		echo json_encode($arr=array('ipAddress'=>$data['ipAddress']));	
		}	
