...
class Admin extends CI_Controller {
	...
	public function page($content=null){
		...
		$data=array('title'=>'Dashboard | Backoffice',
		...
		$arr = array();
			try{
				...
				foreach($sql as $key=>$value){
					$query ="SELECT * FROM m_submenu WHERE menu_id='".$value['id']."' AND unit_id='".$this->Auth['unit']."' AND active='Y' ORDER BY urutan";
					$sql2 =R::getAll($query);
					$sub = array();
					foreach($sql2 as $i=>$val){							
						...
						$arr['menu'][]=array('id'=>$value['id'],
									...
									'lbl_count'=>R::count("m_submenu","menu_id= ? AND unit_id='".$this->Auth['unit']."' AND active=?",array($value['id'],'Y'))
								);
						}				
				}catch (Exception $e) {
					$arr['error'] =true;
	...
	public function create_menu($id) {
		...
		try {
			if ($this->input->post()) {
				$cekpoint = R::findOne('m_menu', "role_id= ?", array($id));
				if (!$cekpoint) {
					$value = '';
					$menu = array("Data Master", "Applikasi", "Report", "Pengaturan");
					for ($i = 0; $i < 4; $i++) {
						$web = R::dispense('m_menu');
						$value = $menu[$i];
						$web->role_id = $id;
						$web->nama_menu = $value;
	...
	private function add_log($action){
			...
			$user = R::load('m_user',$this->Auth['user']);
			$log->user_id = $user->id;
			$log->ipaddress = $this->Auth['ipaddress'];
			$log->action = $action;
	...
	public function create($table) {		
		...
		echo json_encode($arr);
		$this->add_log('CREATE Data Ke Table (' . $table . ') Dengan ID Table ' . $arr['id'] );
		}
			...
			try{
				...
				$sWhere = "";
					if ($this->input->get('aSearch') !== false && $this->input->get('aSearch') != "" )
					{
						...
						$sWhere .= ') ORDER BY id DESC LIMIT 0,5';
	...
	public function get_editable_select(){
			...
			$option = $this->input->get('opt');
			$val = $this->input->get('val');
			if($option){
				$res = R::find($table,$option."=?",array($val));
			}else {
				$res = R::findAll($table);
			}
			$ret = array();
	...
	public function update($table) {
			...
			try{	
				...
				if($this->input->post()){
					...
					foreach ($this->input->post() as $column => $value){
							...
							$ret['code'] =100;
							$ret['id'] =$id;
							$ret['message'] ='Update Data Berhasil...'.$id;
					}else{
						$ret['error'] =true;
						$ret['code'] =101;
			...
			echo json_encode($ret);	
			$this->add_log("Pesan (".$ret['message'].') Melakukan Update Ke Table (' . $table . ') Dengan ID ' . $id );
		} 
	...
	public function update_grid($table) {
			...
			$this->add_log('Update Data ke table (' . $table . ') ID ' . $id .' Melalui DataGrid');
		}	
	public function delete($table, $id) {
			...
			if($bean){
				R::trash($bean);
				$arr['error']=false;
				$arr['message']='Delete Data Berhasil..';
				$arr['code']=200;
			}else{
				$arr['error']=true;
				$arr['message']=$del.'Delete Data Gagal..';
				$arr['code']=200;
			}
			echo json_encode($arr);
	...
	public function create_data_pegawai($table){
			...
			if($this->input->post())
			{
				$cekpoint = R::findOne($table,'sosial_id=?',array($this->input->post('sosial_id')));
				if(!$cekpoint){
					foreach ($this->input->post() as $column => $value) {
						$bean->$column = $value;
						}
					...
					$arr['code']= 200;
					$arr['error']= false;
					$arr['message']= "Berhasil Menyimpan Data";
				}else {
					$arr['code']= 100;
					$arr['error']= true;
					$arr['message']= "Gagal Menyimpan Data Atau Duplikat Data Input";	
				}
			}
			echo json_encode($arr);
		}
	...
	public function edit_upload_foto(){
		...
		$ret = array();
			...
			if($this->upload->do_upload('file')){
				...
				$resize = $this->resizeImage($uploadPath,$nama,$hasil->sizeH);
						...
						for($i=0; $i<$cpt; $i++)
								{           
									...
									if($this->upload->do_upload('file')){
										...
										$drop->keterangan = $hasil->keterangan;
	...
	// Resize image
	public function resizeImage($path,$filename,$size=null)
		{
					...
					if(!empty($_FILES['file']['name'])){							
						...
						for($i=0; $i<$cpt; $i++)
								{           
									...
									$uploadPath = FCPATH.str_replace("+","/",$hasil->path).'/';
									$config['upload_path'] = $uploadPath;
									$config['allowed_types'] = 'pdf|xls|doc|csv';
									$config['encrypt_name'] = TRUE;
									...
									if($this->upload->do_upload('file')){
										...
										$drop = R::dispense($tb);
										$drop->name = $hasil->name;
										$drop->link = str_replace("+","/",$hasil->path).'/'.$nama;
										$drop->type =$type;				
										$drop->date = $date;
										$drop->upload_id= $hasil->idUpd;
										$drop->user_id= $hasil->id;
										R::store($drop);
									}else{
										$ret['error'] =true;
	...
	public function json_load_data(){
		...
		try{
			...
			$config = "";
			if($ctgr){
				$MySql=R::getAll('SELECT * FROM '.$table.' WHERE '.$ctgr.'='.$id.' ORDER BY id ASC LIMIT '.$start.','.$limit.'');
				$config = R::count($table,"'.$ctgr.'=?",array($id));
			}else{
				$MySql=R::getAll('SELECT * FROM '.$table.' ORDER BY id ASC LIMIT '.$start.','.$limit.'');
	...
	public function json_load_page(){
		R::setStrictTyping(false);
		$arr=array();
		try{
			$table = $this->input->get('table');
	...
	public function get_html_menu(){
		$arr="";
		try{
			$path = $this->input->post('html');
			$path = str_replace("-","/",$path);
			
			$dir = FCPATH . 'application/views/'.$path.'.php';
			if(file_exists($dir)){
				$arr = $this->load->view($path);
				}
		}catch(Exception $e){
			$arr =$e->getMessage();
		}
	...
	public function user_verifikasi(){
		...
		try{
			...
			if($id){
				$user = R::load('m_user',$id);
				$arr['use_nama']=$user->nama;
				$arr['use_email']=$user->email;
				$arr['use_ver']=$user->verifikasi;
				$arr['use_employ']=$user->employ_id;
				$arr['use_date']=$user->date;
				$arr['use_updtime']=$user->updtime;
				$arr['use_status']=$user->status;
				$veri = R::findOne('t_verifikasi_user','email=?',array($user->email));
				if($veri){
					$arr['val_date']=$veri->date;
					$arr['val_status']=$veri->status;
					$arr['val_email']=$veri->email;
					$arr['val_updtime']=$veri->updtime;
				}else{
					$arr['val_status']=false;
				}
	...
	public function send_verifikasi(){
		$arr = array();
		R::setStrictTyping(false);
		...
		try{
			if($email){
				...
				$dump->token = $link;
				$dump->date =$date;
				$sending = R::store($dump);
				if($sending){
					$this->sendemail->email_verifikasi($email,$nama,'Verifikasi Email',$link);
					$arr['error'] =false;
					$arr['message']="Berhasil Mengirim Email..";
				}
			}else{
				$arr['error'] =true;
				$arr['message']="Email Tidak Di temukan";
			}
		}catch (Exception $e){
			$arr['error'] =false;
			$arr['message'] =$e->getMessage();
	...
	// SIKONTRAS 
	public function send_data_pengaduan(){
		R::setStrictTyping(false);
		$ret = array();
		$date =R::isoDateTime();
			try{
			...
			if($this->upload->do_upload('file')){
				...
				$resize = $this->resizeImage($uploadPath,$nama);
					...
					$temp = str_replace("+","/",$hasil->path).'/'.$nama;
					$image_data = file_get_contents($temp);
					$image_token = base64_encode($image_data); 
					$id=base64_decode($hasil->id);
					$tb=str_replace("/","_",$hasil->tbL);
					...
					$drop->noaduan =$hasil->noaduan;
					$drop->jenis =$hasil->jenis;
					$drop->perbaikan =$hasil->perbaikan;
					$drop->deskripsi =$hasil->deskripsi;
					$drop->user_id =$this->Auth['user'];
	...
	public function get_json_data_aduan(){
			$arr=array();
			try{
				$id = $this->input->post('id');
				if($id){
				...
				$MySql = "";
					if ($this->input->get('aSearch') != false && $this->input->get('aSearch') != "" )
						...
						for ( $i=0 ; $i<count($aColumns) ; $i++ )
							...
							$sWhere = substr_replace( $sWhere, "", -3 );
							$sWhere .= ') ORDER BY id DESC LIMIT 0,10';
							
							if($this->input->get('aSearch') != false && $this->input->get('fild') != false )
								{
									$MySql= R::getALL('SELECT * FROM '.$table.' WHERE '.$this->input->get('fild').'= ? AND '.$sWhere,array($this->input->get('id')));
								}else {
									$MySql= R::getALL('SELECT * FROM '.$table.' WHERE '.$sWhere.';');
								}
						}
	...
	public function create_item_paket(){
		$ret = array();
		$date = R::isoDateTime();
		$id = 0;
		R::begin();
			...
			$hasil = json_decode(base64_decode($data));
					foreach ($hasil->dataTable as $key => $value) 
							{
							$paket = R::findOne('b_product_paketdetail','kode= ? AND paket_id=?', array($value[1],$id));
							if($paket){
								$paket->qty += $value[3];
								$paket->kode = $value[1];
								$paket->deskripsi = $value[3];	
								$paket->paket_id = $value[4];
								$paket->grup_id = $value[5];
								$paket->item_id = $value[6];
								R::store($paket);
							}else{
								$detail = R::dispense('b_product_paketdetail');
								$detail->kode = $value[1];
								$detail->nama = $value[2];
								$detail->deskripsi = $value[3];	
								$detail->paket_id = $value[4];
								$detail->grup_id = $value[5];
								$detail->item_id = $value[6];
								R::store($detail);
							}
						}	
					$ret['message']= "Membuat Data Paket Berhasil...";
					$ret['code']= '100';
					$ret['error']= false;
				R::commit();
			}catch (Exception $e)
	...
	/* Tools Divisi Kuliner*/
	public function update_harga($table) {
		R::setStrictTyping(false);
		$ret = array();
		$id = $this->input->post('kode');
		$harga = $this->input->post('harga');
		$edit = R::findOne($table,'kode=?',array($id));
		if($edit){
			$edit->harga_jual = $harga;
			$arr['id'] = R::store($edit);
			$arr['message']= "Edit Data Harga Berhasil...";
			$arr['code']= '100';
			$arr['error']= false;
		}else{
			$arr['message']= "Edit Data Harga Berhasil...";
