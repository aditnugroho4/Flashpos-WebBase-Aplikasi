	...
	public function detail($slug=null){		
		...
		if($slug==null){			
			...
			if($hasil){
				$data=array('user'=>R::load('m_user_member',$this->Auth['user']),
					'Menu'=>$this->get_data_menu(),
					'content'=>str_replace('-','/','portal-menu-product-html-h_Blog_detail'),
		...
		try{
			...
			if($MySql){
				foreach($MySql as $i=> $key){	
						$arr['data'][]= array('items'=>$key,'user'=>R::load('m_user',$key['user_id'])->nama,'seo'=>R::load('m_seo',$key['link_id'])->export(),'ctgr'=>R::load('w_post_ctgr_blog',$key['ctgr_id'])->nama);
					}
					if($page == 0){
						$page =1;
					}
